...
function createTextSprite(text: string, size: 'large' | 'medium' | 'small'): THREE.Sprite {
  ...
  const fontSize = size === 'large' ? 24 : size === 'medium' ? 16 : 11;
  canvas.width = 256;
  canvas.height = 48;
  ...
  ctx.fillStyle = size === 'large'
    ? 'rgba(120, 145, 180, 0.5)'
    : size === 'medium'
      ? 'rgba(100, 125, 155, 0.35)'
      : 'rgba(85, 110, 140, 0.25)';
...
export default function EarthMesh() {
  ...
  const gridLineObjects = useMemo(() =>
    gridLines.map(l => {
      ...
      const mat = new THREE.LineBasicMaterial({ color: '#141e35', transparent: true, opacity: 0.15 });
  ...
  const continentLineObjects = useMemo(() =>
    continentPoints.map(pts => {
      ...
      const mat = new THREE.LineBasicMaterial({ color: '#3d6a9e', transparent: true, opacity: 0.55 });
      return new THREE.Line(geom, mat);
    }),
    [continentPoints]
  );
  ...
  const borderLineObjects = useMemo(() =>
    borderLines.map(pts => {
      const geom = new THREE.BufferGeometry().setFromPoints(pts);
      const mat = new THREE.LineBasicMaterial({ color: '#2a4570', transparent: true, opacity: 0.25 });
      return new THREE.Line(geom, mat);
    }),
    [borderLines]
  );

  // Create filled continent areas using meshes
  const continentFills = useMemo(() => {
    // Simple hemisphere fill approach - create subtle continent shading
    return continentPoints.map(pts => {
      if (pts.length < 3) return null;
      const shape = new THREE.Shape();
      // Use screen-space projection isn't practical, instead just show outlines
      return null;
    }).filter(Boolean);
  }, [continentPoints]);
  ...
  return (
    <group ref={groupRef}>
      {/* Globe sphere - subtle gradient effect */}
      <mesh>
        <sphereGeometry args={[1.99, 80, 80]} />
        <meshBasicMaterial color="#060b14" transparent opacity={0.98} />
      </mesh>
      {/* Subtle land mass fill */}
      <mesh>
      ...
      <mesh>
        ...
        <meshBasicMaterial color="#1e3a5f" transparent opacity={0.02} side={THREE.BackSide} />
      </mesh>
      <mesh>
        <sphereGeometry args={[2.15, 64, 64]} />
        <meshBasicMaterial color="#1a365d" transparent opacity={0.015} side={THREE.BackSide} />
      </mesh>
      <mesh>
        <sphereGeometry args={[2.25, 64, 64]} />
        <meshBasicMaterial color="#172554" transparent opacity={0.01} side={THREE.BackSide} />
      </mesh>
    </group>
  );
}
